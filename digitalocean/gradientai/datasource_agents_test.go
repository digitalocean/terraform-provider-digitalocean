...
resource "digitalocean_gradientai_agent" "bar" {
  name        = "%s"
  description = "%s"
  instruction = "%s"
  model_uuid  = "%s"
...
data "digitalocean_gradientai_agents" "result" {
  filter {
    key    = "name"
    values = ["%s"]
  }
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.digitalocean_gradientai_agents.result", "agents.#", "1"),
					resource.TestCheckResourceAttr("data.digitalocean_gradientai_agents.result", "agents.0.name", name1),
					resource.TestCheckResourceAttrPair("data.digitalocean_gradientai_agents.result", "agents.0.id", "digitalocean_domain.foo", "id")),
			},
			{
				Config: resourcesConfig,
			},
