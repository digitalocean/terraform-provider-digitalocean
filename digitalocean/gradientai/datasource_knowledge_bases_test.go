...
resource "digitalocean_gradientai_knowledge_base" "test1" {
  name                 = "%s"
  project_id           = "%s"
  region               = "%s"
...
resource "digitalocean_gradientai_knowledge_base" "test2" {
  name                 = "%s"
  project_id           = "%s"
  region               = "%s"
  embedding_model_uuid = "%s"
...
data "digitalocean_gradientai_knowledge_bases" "all" {}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_knowledge_bases.all", "knowledge_bases.#"),
				),
			},
		},
	})
