...
func TestAccDataSourceDigitalOceanAgent_CompleteConfiguration(t *testing.T) {
	...
	description := "Test GradientAI agent for acceptance testing"
	...
	dataSourceConfig := `
data "digitalocean_gradientai_agent" "foobar" {
  agent_id = digitalocean_gradientai_agent.foo.agent_id
}`
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDataSourceDigitalOceanAgentExists("data.digitalocean_gradientai_agent.foobar", &agent),
					resource.TestCheckResourceAttr(
						"data.digitalocean_gradientai_agent.foobar", "name", name),
					resource.TestCheckResourceAttr(
						"data.digitalocean_gradientai_agent.foobar", "description", description),
					resource.TestCheckResourceAttr(
						"data.digitalocean_gradientai_agent.foobar", "instruction", instruction),
					resource.TestCheckResourceAttr(
						"data.digitalocean_gradientai_agent.foobar", "model_uuid", model_uuid),
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_agent.foobar", "agent_id"),
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_agent.foobar", "uuid"),
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_agent.foobar", "created_at"),
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_agent.foobar", "updated_at"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDataSourceDigitalOceanAgentExists("data.digitalocean_gradientai_agent.foobar", &agent),
					resource.TestCheckResourceAttr(
						"data.digitalocean_gradientai_agent.foobar", "name", agentName),
					resource.TestCheckResourceAttr(
						"data.digitalocean_gradientai_agent.foobar", "tags.#", "1"),
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_agent.foobar", "agent_id"),
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_agent.foobar", "uuid"),
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_agent.foobar", "created_at"),
					resource.TestCheckResourceAttrSet("data.digitalocean_gradientai_agent.foobar", "updated_at"),
...
func testAccCheckDataSourceDigitalOceanAgentExists(n string, agent *godo.Agent) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		foundAgent, _, err := client.GradientAI.GetAgent(context.Background(), agentID)
		if err != nil {
			return err
		}
...
func testAccCheckDataSourceDigitalOceanAgentConfig_complete(name, description, instruction, model_uuid, project_id, region string) string {
	return fmt.Sprintf(`
resource "digitalocean_gradientai_agent" "foo" {
  name        = "%s"
  instruction = "%s"
  description = "%s"
  model_uuid  = "%s"
...
func TestAccDataSourceDigitalOceanAgentVersions_Lifecycle(t *testing.T) {
	...
	const dsCfg = `
data "digitalocean_gradientai_agent_versions" "versions" {
  agent_id = digitalocean_gradientai_agent.foo.id
}`
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
...

			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.digitalocean_gradientai_agent_versions.versions",
						"agent_versions.#", "1"),
					checkExactlyOneApplied("data.digitalocean_gradientai_agent_versions.versions"),
				),
			},
			{
