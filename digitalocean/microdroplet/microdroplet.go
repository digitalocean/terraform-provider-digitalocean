...
var httpProtocolValues = []string{
	...
	string(godo.MicroDropletHTTPProtocolHTTPS),
...
func microDropletResourceSchema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"http_protocol": {
			...
			Optional:     true,
			ForceNew:     true,
			Description:  "HTTP protocol: 'http', 'https', or 'http2'",
		...
		"auto_resume": {
			...
			Optional:    true,
			Computed:    true,
			Description: "Whether the MicroDroplet should auto-resume on request",
		},
		"tags": tag.TagsSchema(),
		"state": {
			Type:             schema.TypeString,
			...
			Description:      "Desired lifecycle state: 'running' or 'paused'. Changes are applied via PATCH.",
...
func microDropletDataSourceSchema() map[string]*schema.Schema {
	...
	// tags: datasource-flavored (Computed set of strings)
	base["tags"] = tag.TagsDataSourceSchema()
	return base
}
